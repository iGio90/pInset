...
InsetEngine.prototype.scalePixels = function(extractedData, targetWidth, targetHeight) {
   ...
   // Scale mask if circular
   var scaledMask = null;
   if (extractedData.isCircular && extractedData.mask) {
      scaledMask = new Float32Array(targetWidth * targetHeight);
      this.bicubicInterpolate(extractedData.mask, srcWidth, srcHeight,
                               scaledMask, targetWidth, targetHeight);
      // Clamp mask values to [0, 1] to preserve anti-aliasing
      for (var i = 0; i < scaledMask.length; i++) {
         if (scaledMask[i] < 0) scaledMask[i] = 0;
         else if (scaledMask[i] > 1) scaledMask[i] = 1;
      }
   }
 ...
 * @param {String} shape - "Rectangular" or "Circular"
 */
InsetEngine.prototype.drawConnectionLine = function(image, sourceRect, insetRect, color, thickness, shape) {
   ...
   if (shape === "Circular") {
      ...
      // Fixed Left/Right connections
      var srcLeftX = Math.round(srcCenterX - srcR);
      var srcLeftY = Math.round(srcCenterY);
      
      var srcRightX = Math.round(srcCenterX + srcR);
      var srcRightY = Math.round(srcCenterY);
      
      var insLeftX = Math.round(insCenterX - insR);
      var insLeftY = Math.round(insCenterY);
      
      var insRightX = Math.round(insCenterX + insR);
      var insRightY = Math.round(insCenterY);
...
/**
 * Bresenham's line algorithm with thickness (optimized).
 * For alpha < 1.0, uses row-based buffered operations to avoid per-pixel API calls.
 */
InsetEngine.prototype.bresenhamLine = function(image, x0, y0, x1, y1, color, thickness) {
   var alpha = (color.a !== undefined) ? color.a : 1.0;
   var halfThick = thickness >> 1;  // Faster than Math.floor(thickness / 2)
   
   // For semi-transparent lines with thickness, collect pixels by row then process with buffers
   if (alpha < 0.99 && thickness > 1) {
      // Collect pixels organized by row: { rowY: [x1, x2, x3, ...] }
      var rowPixels = {};
      var minY = image.height, maxY = 0;
      ...
      while (true) {
         // Add all pixels in the thickness square
         var startY = cy - halfThick;
         var endY = startY + thickness;
         var startX = cx - halfThick;
         var endX = startX + thickness;
         
         for (var ty = startY; ty < endY; ty++) {
            if (ty >= 0 && ty < image.height) {
               if (!rowPixels[ty]) {
                  rowPixels[ty] = {};
               }
               for (var tx = startX; tx < endX; tx++) {
                  if (tx >= 0 && tx < image.width) {
                     rowPixels[ty][tx] = true;
                  }
               }
               if (ty < minY) minY = ty;
               if (ty > maxY) maxY = ty;
      ...
      // Process rows with buffered operations (optimization)
      var colors = [color.r, color.g, color.b];
      var channels = Math.min(3, image.numberOfChannels);
      var oneMinusAlpha = 1.0 - alpha;
      
      for (var row = minY; row <= maxY; row++) {
         var xSet = rowPixels[row];
         if (!xSet) continue;
         
         // Find min/max x for this row
         var minX = image.width, maxX = 0;
         for (var xKey in xSet) {
            var xVal = parseInt(xKey);
         ...
         var rowRect = new Rect(minX, row, maxX + 1, row + 1);
         var rowWidth = maxX - minX + 1;
         ...
         for (var c = 0; c < channels; c++) {
            ...
            for (var col = 0; col < rowWidth; col++) {
               if (xSet[minX + col]) {
                  buffer[col] = colorVal * alpha + buffer[col] * oneMinusAlpha;
               }
            }
   ...
   // Fast path for opaque or thin lines
   var dx = x1 - x0;
   var dy = y1 - y0;
   if (dx < 0) dx = -dx;
   if (dy < 0) dy = -dy;
   ...
   var useFillBox = thickness > 1;
   
   while (true) {
      if (useFillBox) {
         var rect = new Rect(x0 - halfThick, y0 - halfThick, 
                             x0 - halfThick + thickness, y0 - halfThick + thickness);
         this.fillBox(image, rect, color);
      } else {
         // Single pixel case with alpha blending
         if (x0 >= 0 && x0 < image.width && y0 >= 0 && y0 < image.height) {
            var colors = [color.r, color.g, color.b];
            var channels = Math.min(3, image.numberOfChannels);
            for (var c = 0; c < channels; c++) {
               if (alpha >= 0.99) {
                  image.setSample(colors[c], x0, y0, c);
               } else {
                  var existing = image.sample(x0, y0, c);
                  image.setSample(colors[c] * alpha + existing * (1.0 - alpha), x0, y0, c);
               }
            }
         }
      }
...
InsetEngine.prototype.drawSourceIndicator = function(image, sourceRect, color, thickness, shape) {
   ...
   if (shape === "Circular") {
      ...
      // Rectangular path - already optimized with fillBox
      // Top
      var topRect = new Rect(
         sourceRect.x0 - halfThick, 
         sourceRect.y0 - halfThick, 
         sourceRect.x1 + halfThick, 
         sourceRect.y0 - halfThick + thickness
      );
      this.fillBox(image, topRect, color);
      
      // Bottom
      var bottomRect = new Rect(
         sourceRect.x0 - halfThick, 
         sourceRect.y1 - halfThick, 
         sourceRect.x1 + halfThick, 
         sourceRect.y1 - halfThick + thickness
      );
      this.fillBox(image, bottomRect, color);
      
      // Left
      var leftRect = new Rect(
         sourceRect.x0 - halfThick, 
         sourceRect.y0 + halfThick,
         sourceRect.x0 - halfThick + thickness, 
         sourceRect.y1 - halfThick
      );
      this.fillBox(image, leftRect, color);
      
      // Right
      var rightRect = new Rect(
         sourceRect.x1 - halfThick, 
         sourceRect.y0 + halfThick, 
         sourceRect.x1 - halfThick + thickness, 
         sourceRect.y1 - halfThick
      );
      this.fillBox(image, rightRect, color);
   }
};
