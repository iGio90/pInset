      ...
      if (params.drawConnectionLine !== false && dialog.hasDrawnRegion) {
         ...
         if (isCircularInset) {
            ...
            var insR = Math.max(1, Math.min(iw, ih) / 2 - offset);
             ...
             // Draw fixed connections from Left-to-Left and Right-to-Right
             // Source Left/Right
             var srcLeftX = indCx - indR;
             var srcLeftY = indCy;
             var srcRightX = indCx + indR;
             var srcRightY = indCy;
             
             // Inset Left/Right (using reduced radius to hide line ends)
             var insLeftX = insCx - insR;
             var insLeftY = insCy;
             var insRightX = insCx + insR;
             var insRightY = insCy;
